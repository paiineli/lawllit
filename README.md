...
| `Lawllit.Site` | `lawllit.com`. portfolio, resume, browser tools and the contact form |
...
resume, work history and nine browser tools that run with no back-end at all: spreadsheet merging, pdf merging, image compression, base64, sql `in` lists, cpf/cnpj, postal code lookup, qr codes and a password generator. the server only ships the page.
